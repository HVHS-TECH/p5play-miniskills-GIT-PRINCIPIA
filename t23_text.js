...
//Fonts
var font;
const FONT_PATH = "../assets/fonts/HussarBoldCondensed-mmrV.otf"
...
function setup() {
	...
	function() {
		...
		let forward = kb.pressing("up");
		let backward = kb.pressing("down");
		let turn_left = kb.pressing("left");
		let turn_right = kb.pressing("right");
...
function SpawnAliens() {
	...
	for (var i = 0; i < NUM_ALIENS; i++) {
		var world_x = random(-cnv_width + 100, cnv_width - 100);

		var world_y = random(-cnv_height + 100, cnv_height - 100);
...
function draw() {
    ...
    textSize(80);
    textAlign(CENTER);
    text('It has been ' + Math.floor(millis() / 1000) + 's since the scene loaded.', 0, 0);
